...
- Paths starting with "__shared__/<owner_id>/" are docs another user has
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
      ...
      { name: "upload_image", description: "Upload an image to your vault and create a vault doc with an AI-generated description. Pass the image as base64-encoded `image_data`. Returns { doc_path, image_url, description, doc_created }. Cloud mode only — not available in local/stdio mode. Requires ANTHROPIC_API_KEY for auto-description (skipped gracefully if not set).", inputSchema: { type: "object", properties: { image_data: { type: "string", description: "Base64-encoded image bytes." }, media_type: { type: "string", enum: ["image/jpeg", "image/png", "image/gif", "image/webp"], description: "MIME type of the image." }, title: { type: "string", description: "Title for the vault doc. Defaults to 'Image YYYY-MM-DD'." }, path: { type: "string", description: "Vault path for the created doc, e.g. images/my-photo.md. Defaults to images/<slug>.md." }, describe: { type: "boolean", description: "Generate an AI description via Claude Vision. Default true." } }, required: ["image_data", "media_type"] } },
    ],
  }));

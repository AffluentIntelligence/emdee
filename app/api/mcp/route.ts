...
import {
  ...
  lintDoc, distillDoc, materializeSubgroup, createChild, addAssociation, uploadImage,
...
- Paths starting with "__shared__/<owner_id>/" are docs another user has
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
      ...
      { name: "upload_image", description: "Store an image from the conversation into the vault. You (Claude) already see the image — describe it yourself and pass the description here. The image is uploaded to Supabase Storage and a vault doc is created with the image embedded and your description as the summary. Returns { doc_path, image_url, doc_created }. Cloud mode only.", inputSchema: { type: "object", properties: { image_data: { type: "string", description: "Base64-encoded image bytes." }, media_type: { type: "string", enum: ["image/jpeg", "image/png", "image/gif", "image/webp"], description: "MIME type of the image." }, description: { type: "string", description: "Your description of the image — what is shown, any visible text, context. Becomes the doc's blockquote summary." }, title: { type: "string", description: "Title for the vault doc. Defaults to 'Image YYYY-MM-DD'." }, path: { type: "string", description: "Vault path for the created doc, e.g. images/my-photo.md. Defaults to images/<slug>.md." } }, required: ["image_data", "media_type"] } },
    ],
  }));
